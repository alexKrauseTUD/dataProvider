...
int main(int argc, char *argv[]) {
    ...
    if (ConnectionManager::getInstance().configuration->get<bool>(MEMCONNECT_DEFAULT_CONNECTION_AUTO_LISTEN)) {
        std::thread([]() -> void {
            ...
            connectionId = ConnectionManager::getInstance().registerConnection(config, bufferConfig, ConnectionType(ConnectionManager::getInstance().configuration->get<uint8_t>(MEMCONNECT_DEFAULT_CONNECTION_TYPE)));
        ...
        std::thread([]() -> void {
            ...
            for (std::string ip : {"172.16.7.118", "172.16.4.69", "172.16.5.5", "172.16.4.33", "172.16.7.108"}) {
